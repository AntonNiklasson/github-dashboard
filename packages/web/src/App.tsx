			...
			<div className="min-h-0 flex-1 overflow-hidden">
				{activeTab === "all" && instances.length > 1 ? (
					...
					<SingleInstanceDashboard
						instance={
							(activeTab === "all" ? instances[0] : instances.find((i) => i.id === activeTab)) ??
							instances[0]
						}
...
interface DashboardSource {
	instances: Instance[];
	prs: { data: PR[]; isLoading: boolean; isFetching: boolean; error: Error | null };
	recentPrs: { data: RecentPR[] };
	reviews: { data: ReviewRequest[]; isLoading: boolean; isFetching: boolean; error: Error | null };
	notifications: {
		data: Notification[];
		isLoading: boolean;
		isFetching: boolean;
		...
		() => [
			{
				...
				bindings: [
					...
					{ key: "G", label: "Bottom", action: () => nav.setFocusIndex(getMaxIdx()) },
					{
						key: "o",
						label: "Open in browser",
						action: () => {
					...
					},
					{
						...
						action: () => {
							const item = getFocused();
							if (item?.url) window.open(item.url.replace(/\/pull\/\d+.*$/, ""), "_blank");
						},
					},
				],
			},
				...
				{prs.isLoading ? (
					...
					<PrList
						...
						onSaveTitle={async (prNumber, title) => {
							...
							queryClient.setQueriesData<PR[]>({ queryKey: ["prs"] }, (old) =>
								old?.map((p) =>
									p.number === prNumber && p.repo === pr.repo ? { ...p, title } : p,
								),
							);
							try {
...
function Column({
	...
	return (
		<div className="flex min-h-0 flex-col overflow-hidden border-r last:border-r-0">
			...
			<div className="min-h-0 flex-1 overflow-y-auto px-4 pt-4 pb-4 scroll-pt-12">{children}</div>
		</div>
	);
}
