...
export function useReviewRequests(instanceId: string) {
  ...
  return useQuery({
    ...
    queryFn: () => {
      const f = freshRef.current;
      freshRef.current = false;
      return api.reviews(instanceId, f);
    },
    refetchInterval: POLL_INTERVAL,
    enabled: !!instanceId,
  });
...
export function useNotifications(instanceId: string) {
  ...
  return useQuery({
    ...
    queryFn: () => {
      const f = freshRef.current;
      freshRef.current = false;
      return api.notifications(instanceId, f);
    },
    refetchInterval: POLL_INTERVAL,
    enabled: !!instanceId,
  });
  ...
  const queries = useQueries({
    queries: instances.map((inst) => ({
      ...
      queryFn: () => {
        const f = freshRef.current;
        freshRef.current = false;
        return api.notifications(inst.id, f);
      },
      refetchInterval: POLL_INTERVAL,
    })),
  });
...
export function useKeyboardNav(
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const maxIdx = Math.max(0, (itemCountsRef.current[curSection] ?? 0) - 1);
