import type { Instance, Notification, PR, RecentPR, ReviewRequest, SearchPR } from "./types";
...
export const api = {
	...
	recentPrs: (instanceId: string, fresh = false) =>
		fetchJson<RecentPR[]>(`/api/${instanceId}/recent-prs${fresh ? "?fresh=1" : ""}`),
	...
	searchPrs: (instanceId: string, q: string) =>
		fetchJson<SearchPR[]>(`/api/${instanceId}/search/prs?q=${encodeURIComponent(q)}`),
	userPrs: (instanceId: string, username: string) =>
		fetchJson<PR[]>(`/api/${instanceId}/users/${encodeURIComponent(username)}/prs`),
	...
	toggleDraft: async (instanceId: string, repo: string, prNumber: number) => {
		...
		const res = await fetch(`/api/${instanceId}/prs/${owner}/${name}/${prNumber}/toggle-draft`, {
			method: "POST",
		});
		if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
	...
	toggleAutoMerge: async (instanceId: string, repo: string, prNumber: number) => {
		const [owner, name] = repo.split("/");
		const res = await fetch(`/api/${instanceId}/prs/${owner}/${name}/${prNumber}/auto-merge`, {
			method: "POST",
		});
		if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
	...
	closePr: async (instanceId: string, repo: string, prNumber: number) => {
		const [owner, name] = repo.split("/");
		const res = await fetch(`/api/${instanceId}/prs/${owner}/${name}/${prNumber}/close`, {
			method: "POST",
		});
		if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
		return res.json();
	...
	updatePrTitle: async (instanceId: string, repo: string, prNumber: number, title: string) => {
		const [owner, name] = repo.split("/");
		const res = await fetch(`/api/${instanceId}/prs/${owner}/${name}/${prNumber}`, {
			method: "PATCH",
			headers: { "Content-Type": "application/json" },
	...
	approvePr: async (instanceId: string, repo: string, prNumber: number) => {
		const [owner, name] = repo.split("/");
		const res = await fetch(`/api/${instanceId}/prs/${owner}/${name}/${prNumber}/approve`, {
			method: "POST",
		});
		if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
		return res.json();
	...
	rerunCi: async (instanceId: string, repo: string, prNumber: number) => {
		const [owner, name] = repo.split("/");
		const res = await fetch(`/api/${instanceId}/prs/${owner}/${name}/${prNumber}/rerun-ci`, {
			method: "POST",
		});
		if (!res.ok) throw new Error(`${res.status} ${res.statusText}`);
		return res.json();
	...
	postComment: async (instanceId: string, repo: string, prNumber: number, body: string) => {
		const [owner, name] = repo.split("/");
		const res = await fetch(`/api/${instanceId}/prs/${owner}/${name}/${prNumber}/comment`, {
			method: "POST",
			headers: { "Content-Type": "application/json" },
