...
async function getCiStatus(client: Octokit, owner: string, repo: string, ref: string): Promise<string> {
	...
	// Check for failures
	const hasStatusFailure = statuses.some((s) => s.state === "failure" || s.state === "error");
	const hasCheckFailure = checkRuns.some((cr) => cr.conclusion === "failure" || cr.conclusion === "timed_out" || cr.conclusion === "cancelled");
